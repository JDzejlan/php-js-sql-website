...
<body>
    ...
    <form method="post" action="send-email.php" class="form-container">
        ...
        <div style="display: flex;">
            ...
            <input type="text" name="phoneNumber" id="phoneNumber" placeholder="Unesite broj telefona" required>
        </div>
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo '<p>Cijena(kom): <strong><span id="cijena' . $row["ProizvodID"] . '">' . $row["cijena_proizvoda"] . '</span> KM</strong></p>';
                echo '<input type="number" id="quantity' . $row["ProizvodID"] . '" name="quantity' . $row["ProizvodID"] . '" min="0" placeholder="Količina" step="1">';
                echo '</div>';
            }
        } else {
